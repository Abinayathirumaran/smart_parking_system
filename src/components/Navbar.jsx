...
function Navbar() {
  ...
  // Redirect admin to /admin-dashboard and regular users to /dashboard
  ...
  return (
    <nav className="flex flex-wrap md:flex-nowrap items-center justify-between gap-4 px-4 md:px-8 lg:px-12 py-4 bg-slate-950 text-gray-300 max-w-7xl mx-auto w-full">
      ...
      <div className="hidden md:flex items-center gap-4 lg:gap-8">
        ...
        <NavLink to="/slotarea" className={({ isActive }) => isActive 
          ? "text-cyan-400 whitespace-nowrap font-semibold"
          : "text-gray-300 hover:text-cyan-400 whitespace-nowrap"
        }>Slots</NavLink>

        {/* Dashboard link redirects based on user role */}
        <a 
          href="/dashboard" 
          onClick={handleDashboardRedirect} 
          className={
            location.pathname.includes("dashboard")
              ? "text-cyan-400 font-semibold whitespace-nowrap cursor-pointer"
              : "text-gray-300 hover:text-cyan-400 whitespace-nowrap cursor-pointer"
          }
        >
          Dashboard
        </a>
        ...
        {/* Hide Favorites from Admin */}
        {user?.role !== "admin" && (
          <NavLink to="/favourites" className={({ isActive }) => isActive
            ? "text-cyan-400 font-semibold flex items-center gap-2 whitespace-nowrap"
            : "text-gray-300 hover:text-cyan-400 flex items-center gap-2 whitespace-nowrap"
          }>
            <FiHeart />Favorites
          </NavLink>
        )}
      ...
      {isOpen && (
        <div className="absolute top-20 left-0 w-full bg-slate-900 flex flex-col items-center justify-center gap-6 py-6 md:hidden z-50">
          ...
          <a 
            href="/dashboard" 
            onClick={(e) => { setIsOpen(false); handleDashboardRedirect(e); }} 
            className={location.pathname.includes("dashboard") ? "text-cyan-400 font-semibold" : "text-gray-300 hover:text-cyan-400"}
          >
            Dashboard
          </a>
          ...
          {user?.role !== "admin" && (
            <NavLink to="/favourites" onClick={() => setIsOpen(false)} className={({ isActive }) => isActive ? "text-cyan-400 font-semibold flex items-center gap-2" : "text-gray-300 hover:text-cyan-400 flex items-center gap-2"}>
              <FiHeart />Favorites
            </NavLink>
          )}
